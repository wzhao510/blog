<template>
  <el-card class="page-container" style="border-radius: 15px; background-color: #e1e1db">
    <template #header>
      <div class="header">
        <span>文章管理</span>
        <div class="extra">
    ...
    <!-- 搜索表单 -->
    <el-form inline style="background-color: #e1e1db">
    ...
    <!-- 文章列表 -->
    <el-table :data="articles" style="width: 100%;" :header-cell-style="{ background: '#e1e1db', color: 'black' }"
              :cell-style="{ background: '#e1e1db', color: 'black' }">
      <el-table-column label="文章标题" width="400" prop="title">
        <template #default="{ row }">
          <el-link type="primary" @click="showArticlePreview(row.id)">{{ row.title }}</el-link>
        </template>
      </el-table-column>
  ...
  <el-drawer v-model="visibleDrawer" :title="drawerTitle" direction="rtl" size="50%">
    ...
    <el-form :model="articleModel" label-width="100px">
      ...
      <el-form-item label="文章内容">
        <div class="editor">
          <quill-editor v-model:content="articleModel.content" theme="snow" content-type="html"></quill-editor>
        </div>
      </el-form-item>
      <el-form-item>
...
const showArticlePreview = (id) => {
  router.push({ name: 'ArticlePreview', params: { id } });
}
</script>
